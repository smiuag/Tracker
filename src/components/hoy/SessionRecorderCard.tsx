...
import type { Nivel1a5, TipoEstudio } from "@/types/common";
...
export function SessionRecorderCard() {
  ...
  const [energia, setEnergia] = useState<Nivel1a5>(3);
  const [concentracion, setConcentracion] = useState<Nivel1a5>(3);
  ...
  return (
    <SectionCard title="Registrar sesión">
      <div className="flex flex-col gap-4">
        <ToggleGroup
          value={[mode]}
          onValueChange={(v) => v[0] && setMode(v[0] as "timer" | "manual")}
          className="w-fit"
        >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <RatingPicker label="Energía" value={energia} onChange={setEnergia} />
          <RatingPicker label="Concentración" value={concentracion} onChange={setConcentracion} />
        </div>

        <PomodoroTimer count={pomodoros} onChange={setPomodoros} />
